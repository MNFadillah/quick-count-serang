...
class Auth extends ANT_Controller {
	...
	public function index()
	{
		if ($this->auth->logged_in())
		{
			...
			else
			{
				...
				$this->_render_page('auth/login.html', $this->data);
			}
		}
	}
	...
	public function change_password()
	{
		...
		$this->form_validation->set_rules('new', $this->lang->line('change_password_validation_new_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'auth') . ']|max_length[' . $this->config->item('max_password_length', 'auth') . ']|matches[new_confirm]');
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['min_password_length'] = $this->config->item('min_password_length', 'auth');
	...
	public function forgot_password()
	{
		...
		if($this->config->item('identity', 'auth') != 'email' )
		{
		...
		if ($this->form_validation->run() == false)
		{
			$this->data['type'] = $this->config->item('identity','auth');
			// setup the input
			$this->data['identity'] = array('name' => 'identity',
				'id' => 'identity',
			);
		...
		{
			$identity_column = $this->config->item('identity','auth');
			...
			$forgotten = $this->auth->forgotten_password($identity->{$this->config->item('identity', 'auth')});
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			$this->form_validation->set_rules('new', $this->lang->line('reset_password_validation_new_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'auth') . ']|max_length[' . $this->config->item('max_password_length', 'auth') . ']|matches[new_confirm]');
			...
			else
			{
				...
				else
				{
					...
					$identity = $user->{$this->config->item('identity', 'auth')};
	...
	public function create_user()
    {
        ...
        $this->form_validation->set_rules('password', $this->lang->line('create_user_validation_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'auth') . ']|max_length[' . $this->config->item('max_password_length', 'auth') . ']|matches[password_confirm]');
	...
	public function edit_user($id)
	{
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			{
				$this->form_validation->set_rules('password', $this->lang->line('edit_user_validation_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'auth') . ']|max_length[' . $this->config->item('max_password_length', 'auth') . ']|matches[password_confirm]');
	...
	public function edit_group($id)
	{
		...
		$readonly = $this->config->item('admin_group', 'auth') === $group->name ? 'readonly' : '';
	...
	public function _render_page($view, $data=null, $returnhtml=false)//I think this makes more sense
	{
		...
		$view_html = $this->smarty->view($view, $this->viewdata, $returnhtml);
